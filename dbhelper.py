...
class DbHelper:
    ...
    def insert(self, wav_path, status):
        sql_cmd = '''INSERT INTO review (wav_path, status)
                     VALUES ("%s", %s) ''' % (wav_path, status)
        try:
            self.cursor.execute(sql_cmd)
    ...
    def get_all_review(self, offset, number):
        sql_cmd = '''SELECT * FROM review ORDER BY id DESC LIMIT %d,%d''' % (offset, number)
        self.cursor.execute(sql_cmd)
...
if __name__ == '__main__':
    ...
    if not db.table_exist():
        db.create_table()
    db.insert('a.wav', 'true')
    db.insert('b.wav', 'false')
